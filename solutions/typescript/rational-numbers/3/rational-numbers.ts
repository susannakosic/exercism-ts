export class Rational {
  ...
  exprational(exp: number): Rational {
    if (exp < 0) {
      return new Rational(this._den ** -exp, Math.pow(this._num, -exp)).reduce();
    } else {
      return new Rational(this._num ** exp, Math.pow(this._den, exp)).reduce();
    }
  }
  expreal(x: number): number {
    return x ** (this._num / this._den);
  }
}